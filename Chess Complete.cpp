...
bishop::bishop(int c, char s) :pieces(c, s){}

bool  bishop::ifpossible(pieces * board[][8], int oldrow, int oldcolm, int newrow, int newcolm, int colour)
{
	int flag = 1;
	if ((oldrow - newrow) == (oldcolm - newcolm))
	{
		for (int i = oldrow - 1, j = oldcolm - 1; i>newrow; i--, j--)
		{
			if (board[i][j] != NULL)
				flag = 0;
		}
	}
	else if ((newrow - oldrow) == (newcolm - oldcolm))
	{
		for (int i = oldrow + 1, j = oldcolm + 1; i<newrow; i++, j++)
		{
			if (board[i][j] != NULL)
	...
	else if ((oldrow - newrow) == (newcolm - oldcolm))
	{
		for (int i = oldrow - 1, j = oldcolm + 1; i>newrow; i--, j++)
		{
			if (board[i][j] != NULL)
	...
	else if ((newrow - oldrow) == (oldcolm - newcolm))
	{
		for (int i = oldrow + 1, j = oldcolm - 1; i<newrow; i++, j--)
		{
			if (board[i][j] != NULL)
...
queen::queen(int c, char s) :pieces(c, s){}

bool  queen::ifpossible(pieces * board[][8], int oldrow, int oldcolm, int newrow, int newcolm, int colour)
{
	int flag = 1;
	if (newcolm == oldcolm)
	{
	...
	else if ((oldrow - newrow) == (oldcolm - newcolm))
	{
		for (int i = oldrow - 1, j = oldcolm - 1; i>newrow; i--, j--)
		{
			if (board[i][j] != NULL)
...
king::king(int c, char s) :pieces(c, s){}

bool  king::ifpossible(pieces * board[][8], int oldrow, int oldcolm, int newrow, int newcolm, int colour)
{
	if ((oldcolm == newcolm - 1 || oldcolm == newcolm + 1) || (oldrow == newrow - 1 || oldrow == newrow + 1))
	{
		return true;
	}
	else
